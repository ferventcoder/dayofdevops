...
class virtualbox::params {
  ...
  $repo_baseurl = $::virtualbox_repo_baseurl ? {
    undef   => $::operatingsystem ? {
      ...
      'Darwin'            => 'http://download.virtualbox.org/virtualbox/',
  ...
  $repo_key_source = $::virtualbox_repo_key_source ? {
    undef   => $::osfamily ? {
      /(RedHat|Debian|Windows|Darwin)/ => 'http://download.virtualbox.org/virtualbox/debian/oracle_vbox.asc',
      'Windows' => 'http://download.virtualbox.org/virtualbox/debian/oracle_vbox.asc',
      'Darwin' => 'http://download.virtualbox.org/virtualbox/debian/oracle_vbox.asc',
  ...
  $dist_package = $::osfamily ? {
    ...
    'Windows' => 'virtualbox-4.3',
    'Darwin' => 'virtualbox-4.3',
    default  => fail("${::osfamily} is not supported by ${module_name}")
  }
